...
import com.test720.wendujiaoyu.CompressImageUtils;
...
import com.test720.wendujiaoyu.utills.GlideImageLoader;
        ...
        switch (what) {
            ...
            case 1:
                if (object.getInteger("code") == 1) {
                    ...
                    if(info.getData().getUserInfo().getHeadimg().isEmpty()){
                        Glide.with(this).load(R.mipmap.touxiang).asBitmap().into(userCicleImage);
                    }else if(info.getData().getUserInfo().getHeadimg().startsWith("Upload/")){
                        Glide.with(this).load(UrlFactory.imagePath + info.getData().getUserInfo().getHeadimg()).asBitmap().
                                placeholder(R.mipmap.touxiang).error(R.mipmap.touxiang).into(userCicleImage);
                    }else {
                        Glide.with(this).load( info.getData().getUserInfo().getHeadimg()).asBitmap().placeholder(R.mipmap.touxiang).error(R.mipmap.touxiang).into(userCicleImage);
                    }

                    if(info.getData().getUserInfo().getNickname().isEmpty()){
