...
import com.test720.wendujiaoyu.baseUi.MBaseAdapter;
...
public class AnswerErrorItemAdapter extends MBaseAdapter<AnswerBean> {
    private List<Integer> integers = new ArrayList<>();
    private SizeUtils sizeUtils;

    public AnswerErrorItemAdapter(List<AnswerBean> list, Activity mActivity, int stats, List<String> errorList) {
        super(list, mActivity, stats, errorList);
    ...
    public View getXView(final int position, View convertView, ViewGroup parent) {
        ...
        if(position > 4)
            viewHolder.layoutBg.setBackgroundResource(R.drawable.shape_top_rmain_list_bg);
        else
            viewHolder.layoutBg.setBackgroundResource(R.drawable.shape_main_left_list_bg);
        ...
        for (int i = 0; i <integers.size() ; i++) {
            ...
            {
                if(i == 0)
                    viewHolder.layoutBg.setBackgroundResource(R.drawable.shape_main_list_bg);
                else
                    viewHolder.layoutBg.setBackgroundResource(R.drawable.shape_top_main_list_bg);
                break;
            }
        }
            ...
            if("1".equals(list.get(position).getType_name()) || "2".equals(list.get(position).getType_name()))
            {
                if(list.get(position).getSelectedIndex() == null)
                {
                    ...
                    for (int i = 0; i < errorList.size(); i++) {
                        if(list.get(position).getId().equals(errorList.get(i)))
                        {
                            count++;
                            break;
                        }
                    ...
                    if(count > 0)
                    {
                        viewHolder.layoutTopicBg.setBackgroundColor(mActivity.getResources().getColor(R.color.blue));
